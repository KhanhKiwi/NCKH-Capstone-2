...
export default function RopeBundle({
  count, isDragOver, isDragOverWrong
}: Props) {
    ...
    >
      ...
      >
        ...
        {count > 0 && Array.from({ length: count }).map((_, i) => {
          ...
          const startX = 170 - totalSpread/2 + i * (totalSpread / Math.max(count-1, 1))
        ...
        {count === 0 && (
          <g>
            ...
            <path
              ...
              fill="none" stroke="#8D6E63"
              strokeWidth="7" strokeLinecap="round"
            />
            {/* Rope highlight */}
            <path
              d="M20,121 Q85,108 170,115 Q255,108 320,121"
              fill="none" stroke="#BCAAA4"
              strokeWidth="2.5" strokeLinecap="round"
              opacity="0.6"
            />
            {/* Knots */}
            <circle cx="20" cy="124" r="7"
              fill="#6D4C41" stroke="#5D4037" strokeWidth="1.5"/>
            <circle cx="320" cy="124" r="7"
              fill="#6D4C41" stroke="#5D4037" strokeWidth="1.5"/>
            {/* Texture dots */}
            {[70, 120, 170, 220, 270].map(x => (
              <circle key={x} cx={x} cy={118}
                r="3" fill="#6D4C41" opacity="0.4"/>
            ))}
          </g>
        )}
          ...
          <g>
            ...
            <path
              d="M20,128 Q60,118 100,122 Q130,118 150,112
                 ...
                 Q280,118 320,128"
              fill="none" stroke="rgba(0,0,0,0.2)"
        ...
        {count >= 3 && count <= 4 && (
          <g>
            {/* Bottom wrap */}
            <ellipse cx="170" cy="116" rx={30 + count*8} ry="8"
              fill="none" stroke="#8D6E63"
            ...
            />
            {/* Upper wrap band */}
            <ellipse cx="170" cy={100 - count*5} rx={20 + count*6} ry="6"
              fill="none" stroke="#795548"
              strokeWidth="5" opacity="0.85"/>
            <circle cx="20" cy="128" r="7"
              fill="#6D4C41" stroke="#5D4037" strokeWidth="1.5"/>
            <circle cx="320" cy="128" r="7"
              fill="#6D4C41" stroke="#5D4037" strokeWidth="1.5"/>
          </g>
        )}
        ...
        {count === 5 && (
          <g>
            {/* Bottom band */}
            <ellipse cx="170" cy="112" rx="55" ry="10"
              fill="none" stroke="#8D6E63"
              strokeWidth="7"/>
            {/* Middle band */}
            <ellipse cx="170" cy="78" rx="45" ry="8"
              fill="none" stroke="#795548"
              strokeWidth="6"/>
            {/* Top band */}
            <ellipse cx="170" cy="46" rx="30" ry="6"
              fill="none" stroke="#8D6E63"
              strokeWidth="5"/>
            ...
            <ellipse cx="170" cy="78" rx="62" ry="55"
              fill="none" stroke="#FFD54F"
